...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!accountPubkey) {
            return NextResponse.json({ error: "Missing accountPubkey parameter" }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: "Failed to retrieve queues" }, { status: 500 });
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!accountPubkey) {
            return NextResponse.json({ error: "Missing accountPubkey in request body" }, { status: 400 });
        }
